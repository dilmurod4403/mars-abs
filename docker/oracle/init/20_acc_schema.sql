...
--   XXXXXXXX — Уникальный код счета (НИББД, 8 xona; реквизит client_code EMAS)
...
CREATE TABLE core_acc_accounts (
    ...
    account_uid       VARCHAR2(8)   NOT NULL,                 -- XXXXXXXX — Уникальный код счета (НИББД, 8 xona)
    seq_number        VARCHAR2(3)   NOT NULL,                 -- NNN — Порядковый номер счета (001..999)

    -- --- Mijoz bog'lanishi (core_cif hali qurilmagan — qattiq FK YO'Q) -----
    client_code       VARCHAR2(5),                            -- код клиента — SIRIUS реквизит (K, size 5)
...
COMMENT ON COLUMN core_acc_accounts.account_uid        IS 'XXXXXXXX — Уникальный код счета (НИББД, 8 xona; account-number segmenti, реквизит client_code EMAS)';
...
COMMENT ON COLUMN core_acc_accounts.client_code        IS 'код клиента — SIRIUS реквизит (K, size 5)';
...
-- НИББД uid bo'yicha hisoblar
CREATE INDEX core_acc_accounts_uid_idx    ON core_acc_accounts (account_uid);
...
--             account_uid, seq) ANIQ imzosi bilan generatsiya; control_key
...
BEGIN
    IF INSERTING THEN
        ...
        --   CMMSS(5) + VVV(3) + account_uid(8) + NNN(3) ; util K ni 9-pozitsiyaga qo'yadi
        IF :NEW.account_number IS NULL THEN
            v_acc := core_acc_util.Generate_Account_Number(
                         ...
                         LPAD(:NEW.account_uid,     8, '0'),   -- i_client — XXXXXXXX (НИББД uid)
